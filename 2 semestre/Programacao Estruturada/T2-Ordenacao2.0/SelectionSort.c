...
int main()
{
  ...
  output_tempo=fopen("Relatorio_Selection_Sort.txt","w");
  if(inputA==NULL)
  {
...
void ordenar(int vetor[],int fim) //BUBBLE SORT
{
  int i,j,maior,aux;
  for(i=0;i<fim-1;i++)
  {
...
void imprimir(int vetor[],int fim)
{
  ...
  if(fim==tamA)
  {
  outputA=fopen("A_Ordenado_Selection_Sort.txt","w");
  ...
  if(fim==tamB)
  {
  outputB=fopen("B_Ordenado_Selection_Sort.txt","w");
  ...
  if(fim==tamC)
  {
  outputC=fopen("C_Ordenado_Selection_Sort.txt","w");
  ...
  if(fim==tamD)
  {
  outputD=fopen("D_Ordenado_Selection_Sort.txt","w");
