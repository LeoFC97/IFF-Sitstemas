#include <stdio.h>
#include <stdlib.h>

#define linha 5
#define coluna 3

void preencher(float matrizA[linha][coluna],float matrizB[linha][coluna])
{
  int i,j;
  printf("Insira os valores para as matrizes\n");
  for(i=0;i<linha;i++)
  {
    for (j=0;j<coluna;j++)
    {
    scanf("%f", &matrizA[i][j]);
    scanf("%f", &matrizB[i][j]);

    }
  }
}
void soma(float matrizA[][coluna],float matrizB[linha][coluna],float matrizC[linha][coluna])
  {
  int i,j;
  for(i=0;i<linha;i++)
  {
    for (j=0;j<coluna;j++)
    {
    matrizC[i][j]=matrizA[i][j]+matrizB[i][j];
    }
  }
}
void imprimir(float matrizC[linha][coluna]) {
  int i,j;
  for(i=0;i<linha;i++)
  {
    for (j=0;j<coluna;j++)
    {
      printf("%.1f\t",matrizC[i][j]);
    }
    printf("\n");
  }
}

int main() {
  float matrizA [linha][coluna];
  float matrizB [linha][coluna];
  float matrizC [linha][coluna];
  preencher(matrizA,matrizB);
  soma (matrizA,matrizB,matrizC);
  imprimir(matrizC);
  return 0;
}
